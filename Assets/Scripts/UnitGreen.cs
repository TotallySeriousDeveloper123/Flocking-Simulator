...
public class UnitGreen : MonoBehaviour
{
    ...
    void startSetting()
    {
        Push(new Vector2(Random.Range(-10, 10), Random.Range(-10, 10)));
    ...
    Vector2 alignment()
    {
        ...
        float count = 0;
        foreach (GameObject other in manager.GetComponent<SimManager>().UnitsGreen)
        {
            if (other == this.gameObject) continue;
            {
                float distance = Vector2.Distance(location, other.GetComponent<UnitGreen>().location);
                if (distance < neighbourLimit)
                {
                    sum = sum + other.GetComponent<UnitGreen>().velocity;
                    count++;
                }
            }
            if (count > 0)
    ...
    Vector2 cohesion()
    {
        ...
        foreach (GameObject other in manager.GetComponent<SimManager>().UnitsGreen)
        {
            if (other == this.gameObject) continue;
            {
                ...
                {
                    sum = sum + other.GetComponent<UnitGreen>().location;
                    count++;
                }
            }
            if (count > 0)
